...
event_response_t sysenter_cb(drakvuf_t drakvuf,drakvuf_trap_info_t* info){
	...
	if(4 == s->reg_size){
		...
		if(reg_rva_32bit != s->rekall_rva_32bit){
			...
			switch(s->format){
				case OUTPUT_CSV:
					printf("[MSR] msr,%" PRIu32 ",0x%" PRIx32 ",0x%" PRIx32 ",0x%" PRIx32 ",%s,%" PRIi32 "\n",info->vcpu,info->regs->cr3,info->regs->msr_lstar,reg_rva_32bit,info->procname,info->userid);
				break;
				default:
				case OUTPUT_DEFAULT:
		...
		if(reg_rva_64bit != s->rekall_rva_64bit){
			...
			switch(s->format){
				case OUTPUT_CSV:
					printf("msr,%" PRIu32 ",0x%" PRIx64 ",0x%" PRIx64 ",0x%" PRIx64 ",%s,%" PRIi64 "\n",info->vcpu,info->regs->cr3,info->regs->msr_lstar,reg_rva_64bit,info->procname,info->userid);
				break;
				default:
				case OUTPUT_DEFAULT:
